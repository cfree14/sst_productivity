...
# Setup figure
figname <- "SFig16_thetas_by_species_latitude.png"
...
for(i in 1:nrow(nspp)){
  ...
  # Plot data
  xmin <- floor1(min(sdata$lat_dd_abs), 10)
  xmax <- ceiling1(max(sdata$lat_dd_abs), 10)
  plot(betaT ~ lat_dd_abs, sdata, bty="n", las=1, xlab="", ylab="", 
       xlim=c(xmin, xmax), ylim=c(-1, 1), 
       cex.axis=0.8, pch=16, col="grey30", main="", cex.main=0.8)
  title(main=comm_name1, line=0.9, cex.main=0.8)
  title(main=bquote(italic(.(spp))), line=0.3, cex.main=0.8)
  
  # Fit and plot Thiel-Sen slope
  tsfit <- mblm(betaT ~ lat_dd_abs, sdata, repeated=F)
  pvalue <- roundf(summary(tsfit)$coefficients[2,4],2)
  lty <- ifelse(pvalue<0.1, 1, 2)
  curve(coef(tsfit)[1] + coef(tsfit)[2]*x, from=xmin, to=xmax, n=100, add=T, lty=lty)
  text(x=xmax, y=0.95, pos=2, labels=paste0("n=", nspp$n[i]), cex=0.7)
  # text(x=xmax, y=0.8, pos=2, labels=bquote("r"^2*"="*.(r2)), cex=0.7)
  text(x=xmax, y=0.8, pos=2, labels=paste0("p=", pvalue), cex=0.7)
  
  # Fit and plot lm
  # lmfit <- lm(betaT ~ lat_dd, sdata)
  # r2 <- format(round(summary(lmfit)$r.squared, 2), nsmall=2)
  # pvalue <- format(round(anova(lmfit)$'Pr(>F)'[1],3), nsmall=3)
  # lty <- ifelse(pvalue<0.1, 1, 2)
  # curve(coef(lmfit)[1] + coef(lmfit)[2]*x, from=xmin, to=xmax, n=100, add=T, lty=lty)
  # text(x=xmax, y=0.95, pos=2, labels=paste0("n=", nspp$n[i]), cex=0.7)
  # text(x=xmax, y=0.8, pos=2, labels=bquote("r"^2*"="*.(r2)), cex=0.7)
  # text(x=xmax, y=0.63, pos=2, labels=paste0("p=", pvalue), cex=0.7)
  
  # Add 0 influence line
  lines(x=c(xmin, xmax), y=c(0,0), lty=3, lwd=0.6)
  
}

# Add x-axis label
ylabel <- expression("SST influence (θ"["i"]*")")
mtext("Latitude (absolute degrees)", outer=T, side=1, adj=0.53, line=0, cex=0.8)
