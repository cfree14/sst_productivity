...
plot(betaT ~ tmax_yr, data, bty="n", las=1, xpd=NA,
     cex.axis=1.1, cex.lab=1.1,
     xaxt="n", xlim=c(0, 100), ylim=c(-1.5,1.5), 
     xlab="Maximum age (yr)", ylab="",
     col=col, pch=1, cex=1.2)
axis(1, at=seq(0, 100, 20), cex.axis=1.1)
lines(x=c(0, 100), y=c(0,0), lty=3, lwd=1.2)
text(x=(0)-(100-0)*0.325, y=1.5, labels="B", font=2, cex=1.4, xpd=NA)
...
text(labels=paste0("n=", n2), x=-0+(100-0)*1.05, y=-1.5, pos=2, cex=1, xpd=NA)
...
plot(1:10, 1:10, type="n", bty="n", las=1,
     cex.axis=1.1, cex.lab=1.1,
     xlim=c(0, 20), ylim=c(-1.5, 1.5), 
     xlab="Mean temperature (°C)", ylab="")
text(x=(0)-(20-0)*0.325, y=1.5, labels="C", font=2, cex=1.4, xpd=NA)
