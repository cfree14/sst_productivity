...
# Setup figure
figname <- "Fig1_thetas_final_null.png"
png(paste(plotdir, figname, sep="/"), width=6, height=4.5, units="in", res=600)
par(mfrow=c(1,2), mar=c(3.3,0.5,1,0.5), mgp=c(2.8,0.8,0))

# Loop through models
models <- c("yr_t", "lin_ar")
model_names <- c("Final model", "Null model")
for(i in 1:length(models)){

  # Load data
  model_obj <- paste0("ramldb_v3.8_spmodel_temp_dpdt_sst_", models[i], ".Rdata")
...
mtext(xlabel, outer=T, side=1, adj=0.52, line=-1.2, cex=1)
