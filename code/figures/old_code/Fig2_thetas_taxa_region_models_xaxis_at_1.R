...
for(i in 1:length(groups)){
  ...
  group1 <- groups[i]
  model_name <- paste0("ramldb_v3.8_spsst_cobe_", group1, ".Rdata")
...
groups1_labels <- c("Large marine\necoregion", "Family", "FAO major\nfishing area", "Order")
cexs <- c(0.7, 0.65, 0.7, 0.7)

# Setup figure
figname <- "Fig2_thetas_taxa_region_models1.png"
...
for(i in 1:length(groups1)){
  ...
  plot(1:10, 1:10, type="n", bty="n", yaxt="n", cex.axis=0.75,
       xaxt="n", xlim=c(-1, 1), ylim=c(1,nrow(vals1)),
       xlab="", ylab="", cex.main=0.8)
  if(i<3){
    axis(1, at=seq(-1, 1, 0.5), labels=F)
  }else{    
    axis(1, at=seq(-1, 1, 0.5), labels=T, cex.axis=0.85)
  }
  ...
  # Add labels
  text(x=-1.7, y=1:nrow(vals1), pos=4, labels=vals1$label, col=vals1$tcolor, cex=cexs[i], xpd=NA)
  
  # Add model
  text(x=1, y=nrow(vals1)-nrow(vals1)*0, labels=groups1_labels[i], adj=c(1,1), font=2, cex=0.85)
