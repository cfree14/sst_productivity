...
# Setup figure
figname <- "SFig15a_thetas_by_behavior.png"
...
for(i in 1:length(vars)){
  ...
  boxplot(data$betaT ~ data[,vars[i]], frame=F, las=2, col="grey80", border="grey30",
          ylim=c(-1.5, 1.5), lty=1, lwd=0.8, cex.axis=0.9,
          xlab="", ylab="", main=titles[i])
  text(x=0.5, y=-1.4, adj=0, labels=labels[i], cex=0.9)
  
  # Sample size and line
  n <- table(data[,vars[i]])
  text(x=1:length(n), y=rep(1.4, length(n)), labels=n, col="grey20", cex=0.9)  
  lines(x=c(0.5,0.5+length(n)), y=c(0,0), lty=3, lwd=1.2)
  
}

# Add axis label
mtext("Temperature influence", outer=T, side=2, adj=0.57, line=0.5, cex=0.8)
