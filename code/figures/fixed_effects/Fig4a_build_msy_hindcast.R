...
# Read model data
stocks_orig <- read.csv(paste(datadir, "ramldb_v3.8_pella_0.20_cobe_merged.csv", sep="/"), as.is=T)

# Read hindcast data
data_orig <- read.csv(paste(datadir, "msy_hindcast_time_series.csv", sep="/"), as.is=T)
...
if(model=="f"){
  stocks_format <- stocks_orig %>% 
    rename(k=k_f, 
           ...
           betaT=betaT_f, 
           betaT_se=betaT_se_f)
  outfile <- "ramldb_v3.8_pella_0.20_cobe_fixed_msy_hindcast.Rdata"
}else{
  stocks_format <- stocks_orig %>% 
    rename(k=k_r, 
           r=r_r,
           betaT=betaT_r, 
           betaT_se=betaT_se_r)
  outfile <- "ramldb_v3.8_pella_0.20_cobe_random_msy_hindcast.Rdata"
}
