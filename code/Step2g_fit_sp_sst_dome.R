...
if(FALSE){
  dyn.unload(paste(tmbdir, dynlib("spmodel_sst_dome"), sep="/"))
  file.remove(paste(tmbdir, c("spmodel_sst_dome.o", "spmodel_sst_dome.dll"), sep="/"))
  compile("spmodel_sst_dome.cpp")
}

# Load TMB code
dyn.load(dynlib("spmodel_sst_dome"))

# Input data and parameter starting values
b0_starts <- log(sapply(stocks, function(x) max(data$tb_sd[data$stockid==x])) * 1.5)
z_starts <- sapply(stocks, function(x) mean(data$sst_t_sd[data$stockid==x])); hist(z_starts)
