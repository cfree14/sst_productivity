...
if(model=="final"){
  ...
  outputfile <- paste0("ramldb_v3.8_spsst_pella_", roundf(p,2), "_", sst, ".Rdata")
}else{
  sst.data <- as.matrix(data[,paste0(sst, "_sst_c_", model, "_sd")])
  outputfile <- paste0("ramldb_v3.8_spsst_pella_", roundf(p,2), "_", sst, "_", model, ".Rdata")
}
...
if(FALSE){
  dyn.unload(paste(tmbdir, dynlib("spmodel_sst_link_pella"), sep="/"))
  file.remove(paste(tmbdir, c("spmodel_sst_link_pella.o", "spmodel_sst_link_pella.dll"), sep="/"))
  compile("spmodel_sst_link_pella.cpp")
}

# Load TMB code
dyn.load(dynlib("spmodel_sst_link_pella"))
...
# Initialization
model <- MakeADFun(data=input.data, parameters=params, random="BetaT", DLL="spmodel_sst_link_pella")
