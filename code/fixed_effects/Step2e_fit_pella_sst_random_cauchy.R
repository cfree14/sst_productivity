...
# Explore cauchy distribution
x0 <- 0
scale <- 1
x <- seq(-2,2, 0.01)
y <- dcauchy(x, location=x0, scale=scale, log=F)
y2 <- 1 / ((pi * scale) * (1 + ((x-x0)/scale)^2))
plot(x, y, type="l")
plot(x, y2, type="l")
...
if(model=="final"){
  ...
  outputfile <- paste0("ramldb_v3.8_pella_", roundf(p,2), "_", sst, "_random_cauchy.Rdata")
}else{
  sst.data <- as.matrix(data[,paste0(sst, "_sst_c_", model, "_sd")])
  outputfile <- paste0("ramldb_v3.8_pella_", roundf(p,2), "_", sst, "_", model, "_random_cauchy.Rdata")
}
...
if(FALSE){
  dyn.unload(paste(tmbdir, dynlib("pella_sst_random_cauchy"), sep="/"))
  file.remove(paste(tmbdir, c("pella_sst_random_cauchy.o", "pella_sst_random_cauchy.dll"), sep="/"))
  compile("pella_sst_random_cauchy.cpp")
}

# Load TMB code
dyn.load(dynlib("pella_sst_random_cauchy"))
...
# Initialization
model <- MakeADFun(data=input.data, parameters=params, random="BetaT", DLL="pella_sst_random_cauchy")
