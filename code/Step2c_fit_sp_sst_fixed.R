...
if(FALSE){
  dyn.unload(paste(tmbdir, dynlib("spmodel_sst_link_fixed"), sep="/"))
  file.remove(paste(tmbdir, c("spmodel_sst_link_fixed.o", "spmodel_sst_link_fixed.dll"), sep="/"))
  compile("spmodel_sst_link_fixed.cpp")
}

# Load TMB code
dyn.load(dynlib("spmodel_sst_link_fixed"))

# Input data and parameter starting values
params <- list(ln_B0=rep(log(1), nstocks),
               ln_r=rep(log(0.4), nstocks),
               BetaT=rep(0.0, nstocks),
               ln_sigmaP=rep(log(0.2), nstocks)) # 0 before, -2.5 based on model fits
...
# Initialization
model <- MakeADFun(data=input.data, parameters=params, DLL="spmodel_sst_link_fixed")
...
outputfile <-"ramldb_v3.8_spsst_fixed.Rdata"
