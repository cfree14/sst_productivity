...
data <- data_orig %>%
  left_join(select(key, stockid, order, family, fao_area, lme), by="stockid") %>% 
  select(assessid, stockid, order, family, fao_area, lme, everything()) %>% 
...
stocks.nls.no <- spfits$stockid[is.na(spfits$r)] # 26 stocks
stocks.w.big.k <- c("WPOLLNSO", "ARGHAKESARG", "GEMFISHSE",
                    "RBRMECS", "SNAPSAUSNSG", "ALBANPAC", "AMPL3LNO", "REXSOLEGA", "MEG8c9a")
...
if(sst=="obs"){
  ...
  outputfile <- paste0("ramldb_v3.8_spsst_cobe_", group, ".Rdata")
}else{
  sst.data <- as.matrix(data[,paste0("cobe_sst_c_", sst, "_sd")])
  outputfile <- paste0("ramldb_v3.8_spsst_cobe_", group, "_", sst,  ".Rdata")
}
...
if(FALSE){
  dyn.unload(paste(tmbdir, dynlib("spmodel_sst_link_group"), sep="/"))
  file.remove(paste(tmbdir, c("spmodel_sst_link_group.o", "spmodel_sst_link_group.dll"), sep="/"))
  compile("spmodel_sst_link_group.cpp")
}

# Load TMB code
dyn.load(dynlib("spmodel_sst_link_group"))
...
# Initialization
model <- MakeADFun(data=input.data, parameters=params, random=c("BetaT", "mu_group"), DLL="spmodel_sst_link_group")
