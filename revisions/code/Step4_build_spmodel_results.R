...
# Read model output
load(paste(outputdir, "ramldb_v3.8_sp_ar1_pella0.55_cobe_lme.Rdata", sep="/"))
...
stats <- data %>%
   group_by(assessid) %>% 
    summarize(n=n(),
              ...
              sst_c_median=median(cobe_sst_c),
...
# Export data
write.csv(final, paste(outputdir, "ramldb_v3.8_sp_ar1_pella0.55_cobe_lme.csv", sep="/"), row.names=F)
