...
stocks.w.small.k <- c("CHTRACCH", "SNROCKPCOAST", "RROCKLOBSTERCRA2")
...
starts <- filter(starts, !stockid%in%c(stocks.w.small.k))
...
#                ln_r=log(rep(0.2, nstocks)),
#                BetaT=rep(0.0, nstocks),
#                ln_sigmaP=log(rep(0.05, nstocks)), 
#                rho=starts$rho,
#                # rho=rep(0, nstocks),
#                mu_T=0.0,
#                ln_sd_T=-1.25,
...
params <- list(ln_B0=log(pmin(starts$k, 1.5)),
               ...
               mu_group=rep(0, ngroups),
               ln_sd_group=-1.25)
# params <- list(ln_B0=log(starts$k),
#                ln_r=log(starts$r),
#                BetaT=rep(0.0, nstocks),
#                ln_sigmaP=log(starts$sigmaP),
#                # rho=starts$rho,
...
# If null models, fix rho to ML estimates
if(sst!="obs"){
  map <- list()
  map[["rho"]] <- rep(NA, nstocks)
  map[["rho"]] <- factor(map[["rho"]])
  # map[["ln_sigmaP"]] <- rep(NA, nstocks)
  # map[["ln_sigmaP"]] <- factor(map[["ln_sigmaP"]])
  # map[["ln_r"]] <- rep(NA, nstocks)
  # map[["ln_r"]] <- factor(map[["ln_r"]])
  # map[["ln_B0"]] <- rep(NA, nstocks)
  # map[["ln_B0"]] <- factor(map[["ln_B0"]])
}


# Initialization
# model <- MakeADFun(data=input.data, parameters=params, random=c("BetaT", "mu_group"), DLL="spmodel_ar1_pella_sst_group", map=map)
