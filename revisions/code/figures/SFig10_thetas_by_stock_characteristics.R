...
# Setup figure
figname <- "SFig10_thetas_by_stock_characteristics.png"
...
for(i in 1:length(traits)){
  ...
  plot(betaT ~ trait, sdata, bty="n", las=1, xpd=NA,
       xlim=c(xmin, xmax), ylim=c(-1.0,1.0), xlab=xlabels[i], ylab="",
       col=col, pch=1)
  lines(x=c(xmin, xmax), y=c(0,0), lty=3)
  
  # Add sample size text (if not 201)
  n <- nrow(sdata)
  if(n!=nrow(data)){text(x=xmax+(xmax-xmin)*0.05, y=-0.97, pos=2, labels=paste0("n=", n), font=2)}
