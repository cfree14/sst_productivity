...
# Setup figure
figname <- "SFig11_thetas_by_life_history.png"
...
for(i in 1:length(traits)){
  ...
  plot(betaT ~ trait, sdata, bty="n", las=1, xpd=NA,
       xlim=c(xmin, xmax), ylim=c(-1,1), xlab=xlabels[i], ylab="",
       col=col, pch=1)
  lines(x=c(xmin, xmax), y=c(0,0), lty=3)
  
  # Add sample size text (if not 201)
  n <- nrow(sdata)
  if(n!=nrow(data)){text(x=xmax+(xmax-xmin)*0.05, y=-.97, pos=2, labels=paste0("n=", n), font=2)}
