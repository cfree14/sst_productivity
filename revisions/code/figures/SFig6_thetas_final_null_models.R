...
# Setup figure
figname <- "SFig6_thetas_final_null_all.png"
png(paste(plotdir, figname, sep="/"), width=6, height=2.5, units="in", res=600)
par(mfrow=c(1,4), mar=c(3.3,0.5,1,0.5), mgp=c(2.8,0.8,0))

# Loop through models
model_objs <- c(file.path(datadir, "ramldb_v3.8_sp_ar1_pella0.55_cobe_lme.Rdata"),
                file.path(datadir1, "ramldb_v3.8_spsst_pella_cobe_lme_n1.Rdata"),
                file.path(datadir1, "ramldb_v3.8_spsst_pella_cobe_lme_n2.Rdata"),
                file.path(datadir1, "ramldb_v3.8_spsst_pella_cobe_lme_n3.Rdata"))
...
for(i in 1:length(model_objs)){
  ...
  text(x=-1.15, y=8, labels=text_pos, pos=4, adj=1, cex=0.7, col="blue")
  text(x=1.15, y=nrow(vals)-8, labels=text_neg, pos=2, adj=0, cex=0.7, col="red")
