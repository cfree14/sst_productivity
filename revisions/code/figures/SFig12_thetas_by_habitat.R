...
# Setup figure
figname <- "SFig12_thetas_by_habitat.png"
...
# Boxplot by habitat
boxplot(betaT ~ habitat, data, las=2, frame=F, ylim=c(-1, 1),
...
text(x=1:8, y=-0.9, labels=n_hab$n, cex=0.8)
text(x=1:8, y=1.0, labels=n_hab$n_pos, cex=0.8, col=ifelse(n_hab$n_pos>0, "blue", "white"))
text(x=1:8, y=0.85, labels=n_hab$n_neg, cex=0.8, col=ifelse(n_hab$n_neg>0, "red", "white"))
text(x=-0.80, y=1, pos=2, labels="A", font=2, cex=1.2, xpd=NA)

# Boxplot by habitat
boxplot(betaT ~ habitat1, data, las=2, frame=F, ylim=c(-1, 1), lty=1,
        ylab="", yaxt="n", cex.axis=0.9, col=tcolor(c("brown4", "navy"), 0.7))
lines(x=c(0.5,2.5), y=c(0, 0), lty=3)
text(x=1:2, y=-0.9, labels=n_hab1$n, cex=0.8)
text(x=1:2, y=1.0, labels=n_hab1$n_pos, cex=0.8, col=ifelse(n_hab1$n_pos>0, "blue", "white"))
text(x=1:2, y=0.85, labels=n_hab1$n_neg, cex=0.8, col=ifelse(n_hab1$n_neg>0, "red", "white"))
text(x=0.5, y=1, pos=2, labels="B", font=2, cex=1.2, xpd=NA)
