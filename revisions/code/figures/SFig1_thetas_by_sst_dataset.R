...
load(paste(datadir, "ramldb_v3.8_sp_ar1_pella0.55_had.Rdata", sep="/"))
hadsst <- results.wide
...
# Read ERSST version
load(paste(datadir, "ramldb_v3.8_sp_ar1_pella0.55_er.Rdata", sep="/"))
ersst <- results.wide
...
for(i in 1:length(results)){
  ...
  plot(1:10, 1:10, type="n", bty="n", yaxt="n", cex.axis=1,
       xlim=c(-1.0, 1.0), ylim=c(1,nrow(vals)),
  ...
  text(x=-1.15, y=8, labels=text_pos, pos=4, adj=1, cex=0.9, col="blue")
  text(x=1.15, y=nrow(vals)-8, labels=text_neg, pos=2, adj=0, cex=0.9, col="red")
...
plot(betaT_cobe ~ betaT_had, data, bty='n', las=1, col="grey50",
     xlim=c(-1.0, 1.0), ylim=c(-1.0, 1.0), cex.axis=0.87, cex.lab=1.3,
...
text(x=1.1, y=-0.9, pos=2, labels=bquote("r"^"2"*" = "*.(r2_format)), cex=0.9)
