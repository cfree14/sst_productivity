...
load(paste(outputdir, "ramldb_v3.8_sp_ar1_pella0.55_cobe_fixed.Rdata", sep="/"))
data_f <- results.wide
rm(hess, input.data, model, nstocks, output, params, problem.stocks, sd, stocks, results.df)

# Read random effects output
load(paste(outputdir, "ramldb_v3.8_sp_ar1_pella0.55_cobe.Rdata", sep="/"))
data_r <- results.wide
rm(hess, input.data, model, nstocks, output, params, problem.stocks, sd, stocks, results.df)
...
# Export data
write.csv(final, paste(outputdir, "ramldb_v3.8_sp_ar1_pella0.55_cobe_re_fe_merged.csv", sep="/"), row.names=F)
