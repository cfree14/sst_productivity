...
# Setup figure
figname <- "FE_Fig1_thetas_fixed_effects.png"
...
plot(1:10, 1:10, type="n", bty="n", yaxt="n", cex.axis=0.8,
     xlim=c(-12, 12), ylim=c(1,nrow(vals)), xaxt="n",
     xlab=expression("SST influence (θ"["i"]*")"), ylab="", main="Fixed effects", cex.main=1)
axis(1, at=seq(-12,12,2), cex.axis=0.8)
...
n_pos <- sum(vals$est_lo>0 & !is.na(vals$est_lo))
n_neg <- sum(vals$est_hi<0 & !is.na(vals$est_hi))
n_neutral <- nrow(vals)-n_pos-n_neg
text_pos <- paste0(n_pos, " stocks\n", "positive")
text_neg <- paste0(n_neg, " stocks\n", "negative")
text(x=-12, y=6, labels=text_pos, pos=4, adj=1, cex=0.7, col="blue")
text(x=12, y=nrow(vals)-8, labels=text_neg, pos=2, adj=0, cex=0.7, col="red")
...
text_pos <- paste0(n_pos, " stocks\n", "positive")
text_neg <- paste0(n_neg, " stocks\n", "negative")
text(x=-2, y=6, labels=text_pos, pos=4, adj=1, cex=0.7, col="blue")
text(x=2, y=nrow(data_r)-8, labels=text_neg, pos=2, adj=0, cex=0.7, col="red")
