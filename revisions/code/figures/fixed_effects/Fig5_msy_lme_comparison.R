...
model_file <- "ramldb_v3.8_sp_ar1_pella0.55_cobe_fixed.Rdata"
hindcast_file <- "ramldb_v3.8_sp_ar1_pella0.55_cobe_fixed_hindcast_10000traj_unorm.Rdata"
stats_f <- build_data(model_file, hindcast_file)

# LME-scale change: random effects 
model_file <- "ramldb_v3.8_sp_ar1_pella0.55_cobe.Rdata"
hindcast_file <- "ramldb_v3.8_sp_ar1_pella0.55_cobe_hindcast_10000traj_unorm.Rdata"
...
# Setup figure
figname <- "FE_Fig5_msy_lme_comparison_random_fixed.png"
...
range(stats$pdiff_f)
range(stats$pdiff_r)
plot(pdiff_f ~ pdiff_r, stats, bty="n", las=1, yaxt="n",
     xlim=c(-40, 20), ylim=c(-120, 60), pch=21, bg="grey80", cex=msy_wts,
     xlab="Percent change in MSY\n(Random effects)",
     ylab="Percent change in MSY\n(Fixed effects)")
axis(2, at=seq(-120,60,30), las=1)

# Zero lines
lines(x=c(0,0), y=c(-120, 60), lty=3)
lines(x=c(-40,20), y=c(0,0), lty=3)

# One-to-one line
lines(x=c(-40,20), y=c(-40,20))

# Sample size
text(x=stats$pdiff_r, y=stats$pdiff_f, labels=stats$n, cex=0.8)

# Label outlier
out <- filter(stats, pdiff_f < -60)
