...
# Read results
data_orig <- read.csv(paste(datadir, "ramldb_v3.8_sp_ar1_pella0.55_cobe_re_fe_merged.csv", sep="/"), as.is=T)
...
write.csv(msys, paste(datadir, "msy_hindcast_time_series_re_fe_comparison.csv", sep="/"), row.names=F)
write.csv(msy_ts, paste(datadir, "msy_hindcast_time_series_overall_re_fe_comparison.csv", sep="/"), row.names=F)
