...
# Read model data
stocks_orig <- read.csv(paste(datadir, "ramldb_v3.8_sp_ar1_pella0.55_cobe_re_fe_merged.csv", sep="/"), as.is=T)

# Read hindcast data
data_orig <- read.csv(paste(datadir, "msy_hindcast_time_series_re_fe_comparison.csv", sep="/"), as.is=T)

# Which to do? 
# r = random; f = fixed
model <- "f"

# Format stocks based on choice
if(model=="f"){
  stocks_format <- stocks_orig %>% 
    rename(k=k_f, 
           r=r_f,
           betaT=betaT_f, 
           betaT_se=betaT_se_f) %>% 
    mutate(betaT_se=ifelse(is.na(betaT_se) , median(betaT_se, na.rm=T), betaT_se),
           betaT_se=ifelse(betaT_se>8, 2, betaT_se))
  outfile <- "ramldb_v3.8_sp_ar1_pella0.55_cobe_fixed_hindcast_10000traj_unorm.Rdata"
}else{
  stocks_format <- stocks_orig %>% 
    rename(k=k_r, 
           r=r_r,
           betaT=betaT_r, 
           betaT_se=betaT_se_r)
  outfile <- "ramldb_v3.8_sp_ar1_pella0.55_cobe_hindcast_10000traj_unorm.Rdata"
}
...
save(stocks, sst,
     ...
     file=paste(datadir, outfile, sep="/"))
