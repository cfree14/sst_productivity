...
datadir1 <- "/Users/cfree/Dropbox/Chris/Rutgers/projects/productivity/models/sst_productivity/output"
...
figname <- "Science_Fig1_thetas_final_null_plus.pdf"
pdf(paste(plotdir, figname, sep="/"), width=6.5, height=4)
# quartz.save( file = file.path(plotdir, figname), type = 'pdf', device=dev.cur(), width=6.5, height=4)
...
# Loop through models
ar1_null <- file.path(datadir, "ramldb_v3.8_sp_ar1_pella0.55_cobe_lme_n1.Rdata")
models <- c(file.path(datadir, "ramldb_v3.8_sp_ar1_pella0.55_cobe_lme.Rdata"),
            file.path(datadir1, "ramldb_v3.8_spsst_pella_cobe_lme_n1.Rdata")) # "ramldb_v3.8_sp_ar1_pella0.55_cobe_lme_n1.Rdata"
...
for(i in 1:length(models)){
  ...
  text(x=0, y=nrow(vals), labels=paste0("\U00B5 = ", format(round(mu_est,2), nsmall=2)), pos=4, cex=0.9, col="grey30") # \U00B5 = μ
  ...
  text(x=-1.05, y=12, labels=text_pos, pos=4, adj=1, cex=0.9, col="blue")
  text(x=1.05, y=nrow(vals)-18, labels=text_neg, pos=2, adj=0, cex=0.9, col="red")
...
# ex_text <- c("Warming \U2191\nproductivity", "Warming \U2193\nproductivity", "Warming has\nno effect") # \U2191 = ↑, \U2193 = ↓
ex_text <- c("Warming increases\nproductivity", "Warming decreases\nproductivity", "Warming has\nno effect") # = ↑, = ↓
...
for(i in 1:length(ex_stocks)){
  ...
  # Add degree labels
  if(i==1){
    text(x=4.5, y=3, pos=2, labels="+1°C", cex=0.9, col=colors[4])
  ...
  # Add letter
  text(x=xmax[i], y=ymax[i], labels=LETTERS[i+2], pos=1, font=2, cex=1.1, offset=0.2)
