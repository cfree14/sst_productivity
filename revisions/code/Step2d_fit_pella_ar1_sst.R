...
if(model=="final"){
  ...
  outputfile <- paste0("ramldb_v3.8_sp_ar1_pella", roundf(p,2), "_", sst, ".Rdata")
}else{
  sst.data <- as.matrix(data[,paste0(sst, "_sst_c_", model, "_sd")])
  outputfile <- paste0("ramldb_v3.8_sp_ar1_pella", roundf(p,2), "_", sst, "_", model, ".Rdata")
}
...
if(FALSE){
  dyn.unload(paste(tmbdir, dynlib("spmodel_ar1_pella_sst"), sep="/"))
  file.remove(paste(tmbdir, c("spmodel_ar1_pella_sst.o", "spmodel_ar1_pella_sst.dll"), sep="/"))
  compile("spmodel_ar1_pella_sst.cpp")
}

# Load TMB code
dyn.load(dynlib("spmodel_ar1_pella_sst"))

# Input data and parameter starting values
# params <- list(ln_B0=rep(log(1.5), nstocks),
#                ln_r=rep(log(0.3), nstocks),
#                BetaT=rep(0.0, nstocks),
#                ln_sigmaP=rep(log(0.15), nstocks),
#                rho=rep(0.3, nstocks),
#                mu_T=0.0,
#                ln_sd_T=-1.25)
...
# Initialization
model <- MakeADFun(data=input.data, parameters=params, random="BetaT", DLL="spmodel_ar1_pella_sst")
