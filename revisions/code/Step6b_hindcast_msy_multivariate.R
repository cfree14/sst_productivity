...
# Read model data
stocks_orig <- read.csv(paste(datadir, "ramldb_v3.8_sp_ar1_pella0.55_cobe_lme.csv", sep="/"), as.is=T)
...
ntrajs <- 10000
betaT_sim <- rmvnorm(n=ntrajs, mean=stocks_orig$betaT, sigma=joint_covmat_betaT)
mean_sim <- apply(betaT_sim, 2, mean)
plot(mean_sim ~ stocks_orig$betaT)
...
# Plot to see
plot(msy ~ year, msy_trajs_df, type="l", las=1, bty="n", xlim=c(1930, 2010), ylim=c(30,40))
...
save(stocks, sst,
     ...
     file=paste(datadir, "ramldb_v3.8_sp_ar1_pella0.55_cobe_lme_msy_hindcast_10000traj_mvnorm.Rdata", sep="/"))
