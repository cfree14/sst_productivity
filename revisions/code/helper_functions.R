...
format_results <- function(results){
  ...
  params_of_interest <- c("B0", "BetaT", "ln_B0", "ln_r", "ln_sigmaP", "r", "sigmaP", "rho")
  ...
  ste_wide1 <- ste_wide %>% 
    rename(k_se=B0, betaT_se=BetaT, ln_k_se=ln_B0, ln_r_se=ln_r, 
           ln_sigmaP_se=ln_sigmaP, r_se=r, sigmaP_se=sigmaP, rho_se=rho)
  ...
  wide <- est_wide1 %>% 
    ...
    mutate(ln_r_lo=ln_r-ln_r_se*1.96,
           ...
           rho_lo=rho-rho_se*1.96,
           rho_hi=rho+rho_se*1.96, 
           r_lo=exp(ln_r_lo),
           r_hi=exp(ln_r_hi),
           k_lo=exp(ln_k_lo),
           k_hi=exp(ln_k_hi),
